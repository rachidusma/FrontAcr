		...
		<v-row>
			...
			<v-col cols="12">
				...
				<v-card outlined class="pa-3">
					...
					<overdueCustomerSection :overduceCustomerID="this.overdueID" :customername="customerNameForCustomerSection" />
				</v-card>
			</v-col>
			<!-- End Customer -->
					...
					<v-dialog v-model="dialog" persistent scrollable max-width="600px">
						...
						<template v-slot:activator="{ on }">
							<div style="border: 1px dashed #aaa;" class="pa-3 mt-3">
								...
								<!-- End Add Text -->
								<template >
									<v-btn color="success lighten-2" dark @click="addRemind" >Add Remind</v-btn>
								</template>
							</div>
						</template>
            ...
            <v-row>
						  <v-col cols="12" class="flex-wrap" v-if="calculations.Rmindflag">
								...
								<v-col  class="align-center d-flex">
									<div class="d-inline-block">
										<p class="my-0 py-0 caption">Total sum to pay</p>
										<b>{{ this.calculations.Total_sum }} kr</b>
                    <p class="caption">This is remind inovice for OCRnumber: {{ this.overdueUser.ocrid }}</p>
									</div>
								</v-col>
              </v-col>
            </v-row>
			...
			<!-- End Product -->
			<overdueTermSectionVue :todatefromDraft="todatefromDraft" :datefromDraft="datesForTermSection" />
			<overdueDeliveryMethods :calculations="calculations" :draggableItems="draggableItems" :invoiceId="invoiceId" />
...
import overdueCustomerSection from "./overdueCustomerSection";
import overdueTermSectionVue from './overdueTermSection.vue';
import overdueDeliveryMethods from "./overdueDeliveryMethods";
...
export default {
    ...
    addRemind() {
      console.log(this.overdueUser.total)
      this.calculations.Rmindflag = true;
      this.calculations.OverdueInterest = this.overdueUser.total * 0.12 / 12;
      this.calculations.Total_sum = this.calculations.OverdueInterest + 60;
    }
	}
};
</script>
